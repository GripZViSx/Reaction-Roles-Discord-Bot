        ...
        banSchema.findOne({ User: interaction.user.id }, async (err, data) => {
            if (data) {
                ...
                if (interaction.options._subcommand !== null && interaction.options.getSubcommand() == "help") {
                    ...
                    return client.embed({
                        title: `<a:diamondss:922169246723739668> Help Panel`,
                        desc: `<:stacx_modmail:1035431454773088256> >>> **Note**\n<:announcement_channel_alt:924661529171869716> **Always Keep The Role Of Task N Above The Roles Which You Want To Place In Menu, The Bot Can't Remind For This**\n\n┏ \`help\` - **Get information about the reaction roles category commands**
┣ \`add\` - **Add a reaction role**
┣ \`delete\` - **Delete a reaction roles category**
┣ \`list\` - **Show all reaction roles categories from this guild**
┣ \`button\` - **Show all reaction roles with buttons**
┗ \`menu\` - **Show all reaction roles in a menu**`,
                        type: 'editreply'
                    }, interaction)
                }
        ...
        if (buttonID[0] == "reaction_button") {
            reactionSchema.findOne({ Message: interaction.message.id }, async (err, data) => {
                ...
                if (interaction.member.roles.cache.get(roleid)) {
                    ...
                    interaction.reply({ content: `<:premio_tick:953933114533609492> Removed <@&${roleid}> role from you.`, ephemeral: true });
                }
                else {
                    interaction.guild.members.cache.get(interaction.user.id).roles.add(roleid).catch(error => { })

                    interaction.reply({ content: `<:premio_tick:953933114533609492> Added <@&${roleid}> role to you.`, ephemeral: true });
                }
            })
        }
    }
                ...
                async (err, data) => {
                    ...
                    for (let i = 0; i < interaction.values.length; i++) {
                        ...
                        if ((i + 1) === interaction.values.length) {
                            interaction.reply({
                                content: `<:premio_tick:953933114533609492> Updated ${roles} the role for you.`,
                                ephemeral: true,
                            });
                        }
                    }
